...
class TestSettings(unittest.TestCase):
    ...
    def test_related_format_validator(self):
        ...
        # test value format
        self.assertRaises(RelatedFormatError, validator.validate, "{'dexterity.localroles.related_parent':''}")
        # empty role list is ok
        self.assertIsNone(validator.validate("{'dexterity.localroles.related_parent':[]}"))
        # test roles
        self.assertRaises(RoleNameError, validator.validate, "{'dexterity.localroles.related_parent':['ReaderR']}")
        # all is ok
    ...
    def test_localroleconfigurationadapter(self):
        ...
        setattr(cls, "static_config", [{"state": "private", "value": "raptor", "roles": ("Reader",), "related": ""}])
        self.assertIsInstance(getattr(cls, "static_config"), list)
        self.assertEqual(len(getattr(cls, "static_config")), 1)
        self.assertDictEqual(
            getattr(cls, "static_config")[0],
            {"related": "", "state": "private", "value": "raptor", "roles": ("Reader",)},
        )

        values = [
            {"state": "private", "value": "raptor", "roles": ("Reader", "Contributor"), "related": ""},
            {"state": "private", "value": "caveman", "roles": ("Reader",), "related": ""},
            {"state": "pending", "value": "caveman", "roles": ("Contributor",), "related": ""},
        ]
        dict_values = {
            "private": {
                "raptor": {"roles": ("Reader", "Contributor"), "rel": ""},
