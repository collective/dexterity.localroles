...
def configuration_change_analysis(event):
    ...
    com_state_set, add_state_set, rem_state_set = compare_lists(
        list(event.old_value.keys()), list(event.new_value.keys())
    )
    if rem_state_set:
    ...
    for st in com_state_set:
        com_princ_set, add_princ_set, rem_princ_set = compare_lists(
            list(event.old_value[st].keys()), list(event.new_value[st].keys())
        )
        if rem_princ_set:
            only_reindex |= set([st])
        ...
        for pr in com_princ_set:
            ...
            com_roles_set, add_roles_set, rem_roles_set = compare_lists(
                event.old_value[st][pr]["roles"], event.new_value[st][pr]["roles"]
            )
